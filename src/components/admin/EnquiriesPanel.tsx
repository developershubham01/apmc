...
export function EnquiriesPanel({ adminKey, onAuthError }: Props) {
  ...
  const gotoPage = (p: number) => {
    ...
    document.getElementById("enquiries-top")?.scrollIntoView({ behavior: "smooth" });
  };
  ...
  return (
    <div id="enquiries-top">
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
        {stats ? (
          <p className="flex items-center gap-2 text-xs font-500 text-ink-600/80">
            ...
            of <span className="font-700 text-navy">{filtered.length}</span> filtered
            enquiries <span className="text-ink-600/60">· {stats.total} total</span>
          </p>
        ) : (
          <span />
        )}
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-xl bg-white px-3 py-1.5 text-xs font-600 text-ink-600 ring-1 ring-border transition hover:text-royal disabled:opacity-60"
        >
      ...
      {pageCount > 1 && (
        ...
        >
          <button
            ...
            disabled={safePage <= 1}
            className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-border bg-white text-ink-600 shadow-sm transition hover:text-royal disabled:opacity-40"
          ...
          {Array.from({ length: pageCount }, (_, i) => i + 1)
            ...
            .map((item, idx) =>
              item === "gap" ? (
                <span key={`gap-${idx}`} className="px-1 text-xs text-ink-600/60">
                  …
                </span>
              ) : (
                <button
                  ...
                  className={cn(
                    "inline-flex h-9 min-w-9 items-center justify-center rounded-xl px-2 text-xs font-700 shadow-sm transition",
                    safePage === item
                      ? "bg-navy text-white"
                      : "border border-border bg-white text-ink-600 hover:text-royal"
                  )}
                >
                  {item}
                </button>
